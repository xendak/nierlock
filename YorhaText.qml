...
Item {
    ...
    implicitWidth: mainText.implicitWidth + (showShadow ? shadowOffset : 0) + (!_typingDone ? _cursorSize + 4 : 0)
    ...
    readonly property string _displayText: {
        ...
        if (glitchEnabled && _glitchIndex >= 0 && _glitchIndex < _charCount) {
            visible = visible.substring(0, _glitchIndex) + _glitchChar + visible.substring(_glitchIndex + 1);
        }
        return visible;
    }
